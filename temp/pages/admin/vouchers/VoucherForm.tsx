...
const VoucherForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Submitting Voucher Payload:", formData);
    if (isEditMode) {
      showToast(`Đã cập nhật mã "${formData.code}"`, 'success');
    } else {
      showToast(`Đã tạo mã mới "${formData.code}"`, 'success');
    }
    navigate('/admin/vouchers');
  };
  ...
  const hasUsage = (formData.used_count || 0) > 0;

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/admin/vouchers')} className="p-2 hover:bg-gray-100 rounded-full text-gray-500">
            <ArrowLeft size={20} />
          </button>
          <div>
        ...
        <div className="flex gap-3">
          <Button variant="secondary" onClick={() => navigate('/admin/vouchers')} className="h-10 px-4 py-2">
            Hủy bỏ
          </Button>
