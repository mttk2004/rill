  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
         <button onClick={() => navigate('/admin/products/create')} className="flex items-center justify-center gap-3 bg-white/80 backdrop-blur-sm p-4 rounded-xl border border-white/60 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group">
         ...
         <button onClick={() => navigate('/admin/artists/create')} className="flex items-center justify-center gap-3 bg-white/80 backdrop-blur-sm p-4 rounded-xl border border-white/60 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group">
         ...
         <button onClick={() => navigate('/admin/vouchers/create')} className="flex items-center justify-center gap-3 bg-white/80 backdrop-blur-sm p-4 rounded-xl border border-white/60 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group">
         ...
         <button onClick={() => navigate('/admin/settings')} className="flex items-center justify-center gap-3 bg-white/80 backdrop-blur-sm p-4 rounded-xl border border-white/60 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        <div className="lg:col-span-1 bg-white/80 backdrop-blur-xl rounded-2xl border border-white/50 shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
                ...
                <Link to="/admin/artists" className="text-xs text-primary hover:underline font-medium">Tất cả</Link>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
           ...
           <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/50 shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                 ...
                 <Link to="/admin/orders" className="text-xs text-primary hover:underline font-medium">Tất cả</Link>
              </div>
              <div className="space-y-3">
                 {pendingOrders.length > 0 ? pendingOrders.map(order => (
                    <div key={order.id} className="flex items-center justify-between p-3 bg-amber-50/50 rounded-xl border border-amber-100 hover:bg-amber-100/80 transition-colors cursor-pointer" onClick={() => navigate(`/admin/orders/${order.id}`)}>
                       <div className="flex items-center gap-3">
                          <div className="w-2 h-12 bg-amber-400 rounded-full"></div>
                          <div>
                             <p className="text-sm font-bold text-gray-900">{order.order_number}</p>
                             <p className="text-xs text-gray-500 mt-0.5 flex items-center gap-1">
                                {formatDate(order.created_at)} • <Users size={10}/> {order.shipping_address.full_name}
                             </p>
                          </div>
                       </div>
           ...
           <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/50 shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                 ...
                 <Link to="/admin/products" className="text-xs text-primary hover:underline font-medium">Quản lý kho</Link>
              </div>
              <div className="space-y-3">
                 {lowStockProducts.length > 0 ? lowStockProducts.map(product => (
                    <div key={product.id} className="flex items-center gap-3 pb-3 border-b border-gray-100 last:border-0 hover:bg-gray-50/50 p-2 -mx-2 rounded-lg transition-colors">
                       ...
                       <button onClick={() => navigate(`/admin/products/${product.id}`)} className="p-2 text-gray-400 hover:text-primary hover:bg-white rounded-lg border border-transparent hover:border-gray-200 shadow-none hover:shadow-sm transition-all">
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/50 shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-100/50 flex items-center justify-between">
          ...
          <button onClick={() => navigate('/admin/orders')} className="text-sm font-medium text-primary hover:underline">Xem tất cả</button>
        </div>
